...
int main(int argc, char **argv) {
  qsub_job_monitor::cargs ap(argc, argv);
  if (ap.help() || argc == 1) {
  ...
  std::string qsub_command = "qsub -N " + job_name + " -o " + logging_prefix +
                             ".output -e " + logging_prefix + ".error -V -q " +
                             qsub_queue + " -l " + resources +
                             " -cwd -S /bin/bash -b y bash " + command_script;
  ...
  if (qsub_job_monitor::exec(qsub_command, &qsub_screen_output)) {
    throw std::runtime_error("unable to execute qsub command: \"" +
                             qsub_command + "\"");
  }
  std::cout << qsub_screen_output << std::endl;
  unsigned job_id = qsub_job_monitor::get_job_id(qsub_screen_output);
  ...
  while (true) {
    ...
    if (seconds_elapsed_since_crashcheck >= crashcheck_interval_in_seconds) {
      ...
      for (; n_crashcheck_retries < crashcheck_attempts;
        ...
        if (qsub_job_monitor::exec("qstat", &qstat_log)) {
          ...
          if ((finder = current_job_ids.find(job_id)) !=
            ...
            qsub_job_monitor::kill_job(job_id);
            if (eqw_resubs >= eqw_resub_limit) {
              std::cout
                  << "ERROR: job \"" << qsub_command << "\" (id " << job_id
                  << ") has Eqw, Eqw resubmission limit reached, terminating"
                  << std::endl;
            } else {
              ++eqw_resubs;
              std::cout << "WARNING: job \"" << qsub_command << "\" (id "
              ...
              job_id = qsub_job_monitor::get_job_id(qsub_screen_output);
            ...
            if (std::filesystem::exists(logging_prefix + ".success")) {
              ...
              sleep(120);
              if (std::filesystem::exists(logging_prefix + ".success")) {
                std::cout << "resolution: job \"" << qsub_command << "\" (id "
                          << job_id
                ...
                if (qsub_job_monitor::exec(qsub_command, &qsub_screen_output)) {
                  throw std::runtime_error(
                      "unable to relaunch qsub command: \"" + qsub_command +
                      "\"");
                }
                std::cout << "resub (job crashed): " << qsub_screen_output
                          << std::endl;
                job_id = qsub_job_monitor::get_job_id(qsub_screen_output);
                seconds_elapsed_since_crashcheck = 0;
                break;
      ...
      if (n_crashcheck_retries >= crashcheck_attempts) {
        throw std::runtime_error(
            "in crashcheck, failed qstat attempts exceeded acceptable "
            "threshold");
      }
    }
  }
