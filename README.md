...
qsub interface for "plco-analysis" project
...
By default, a build process involving a [conda](https://docs.conda.io/en/latest/) environment is supported.
  ...
  - navigate into your project directory (qsub_job_monitor)
  - create the `conda` environment for installation as follows:
  
     `conda env create -f environment.yaml`
  - activate the conda environment:
  
     `conda activate qsub_job_monitor-env`
  ...
  - run `configure`:
	 ...
	 `./configure --with-boost=/path/to/miniconda3/envs/qsub_job_monitor-env --with-boost-libdir=/path/to/miniconda3/envs/qsub_job_monitor-env/lib`
